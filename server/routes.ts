...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/password-reset/request", async (req, res) => {
    try {
      ...
      if (!emailService.isConfigured()) {
        // Retorna apenas deep link para aplicativo quando SMTP não está configurado
        const appLink = `amigomontador://app/reset-password?token=${resetToken}`;
          
        return res.json({ 
          message: "Email não configurado. Use o link do aplicativo para redefinir sua senha:",
          appLink: appLink,
  ...
  app.get("/api/services", async (req, res) => {
    try {
      ...
      .toISOString()}] Buscando serviços para usuário tipo: ${req.user?.userType}`);
  ...
  app.get("/api/services/active", async (req, res) => {
    try {
      ...
      const enhancedServices = await Promise.all(filteredServices.map(async (service) => {
        ...
        - User ${req.user.id} rated: ${hasRated.length > 0}`);
    ...
    try {
      ...
      if (existingApplication) {
        ...
        if (existingApplication.status === 'accepted') {
          ...
          // @ts-ignore - TypeScript strict checking for status comparison
      ...
      try {
        ...
        if (req.user?.userType === 'montador') {
          ...
          if (storeResult.length > 0) {
            ...
            : ${notificationSent ? 'sucesso' : 'falhou'}`);
          }
        } else if (req.user?.userType === 'lojista') {
