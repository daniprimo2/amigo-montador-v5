  ...
  return (
    <div className={`fixed bottom-20 sm:bottom-6 right-6 z-[9999] ${className}`}>
      ...
      <button
        ...
        className={`group relative bg-green-500 hover:bg-green-600 text-white p-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-110 ${isClicked ? 'scale-95' : ''} cursor-pointer z-10`}
      ...
      >
        ...
        <div className={`absolute bottom-full right-0 mb-2 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg whitespace-nowrap transition-opacity duration-200 ${isHovered ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          <div className="text-center">
            ...
            <div className="text-xs text-green-300 mt-1">+55 11 99350-5241</div>
          </div>
          <div className="absolute top-full right-4 w-0 h-0 border-l-4 border-r-4 border-t-4 border-l-transparent border-r-transparent border-t-gray-900"></div>
        </div>
        
        {/* WhatsApp icon overlay - moved inside button */}
        <div className="absolute -top-1 -right-1 bg-white rounded-full p-1 shadow-sm pointer-events-none">
